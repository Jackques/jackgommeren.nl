(function(jQuery) {
	...
	jQuery.fn.stickyNotes.prepareContainer = function(container) {
		...
		if (jQuery.fn.stickyNotes.options.controls) {
			jQuery("#sticky-container").html('<button id="add_note">+</button>');			
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6)  {
            background = $(document.createElement('div')).addClass("background").html('<img src="images/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'Post-it_files/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = $(document.createElement('div')).addClass("background").html('<img src="Post-it_files/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
